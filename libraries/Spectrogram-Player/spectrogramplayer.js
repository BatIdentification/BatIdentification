...
function sp_init() {
  ...
  for(i=0;i<players.length;i++) {
    ...
    if(audioElms.length != 1)
      continue;
    if(player.hasAttribute('image-dimensions')){
      width = $(player).width();
      height = $(player).children("img").eq(0).height();
    }else{
    ...
    if(axisWidth > 0) {
      ...
      if(axisSmoothing != 0)
        divisions = smoothAxis(freqMax-freqMin, divisions, [0], axisSmoothing);
...
function moveSpectrograms(IDs) {
  for(i=0;i<IDs.length;i++) {
    ...
    timeBar = document.getElementsByClassName("sp-timeBar")[id];
    ...
    timeBar.style.left = (viewerWidth/duration) * audio.currentTime + "px"
  }
}
